...
## 1. Optimizations modal — do this FIRST  ·  (mini)
...
## 2. Native values — new toggles in the modal  ·  (mini)
...
- [~] **Native chars / strings** — dropped as a peephole: Char ≡ Scott numeral, so char
      comparison is already the number ops; string *display* is a read-lens follow-up ().
...
## 3. Kernels / FFI — stretch, longer  ·  (full : )
...
- [ ] Spike a MicroHs-style **kernel** mechanism: bind a named combinator to a native JS
      ...
      Generalises §2; could surface real primitives in the Haskell panel.  first.
...
- [ ] **gcd** (`u1Sr43PU`) — Church Euclid is over-budget. ⚠️ Native numbers (§2) do
      **not** help: the solution is raw S/K/I with no named op to intercept (Codex,
      ). gcd needs **kernels** (§3) or a Church abstract interpreter — or it stays
...
## 5. Refactor pass — make the codebase manageable  ·  (mini)
