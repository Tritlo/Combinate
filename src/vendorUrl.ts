 ...
 * MicroHs compiler, its cache) are fetched at runtime, so a bare `/vendor/...` would
 * resolve against the origin root and 404 on a subpath deploy. Resolving
 ...
 *
 * The result is absolute, so it's also safe to hand to a Worker's `importScripts`
 * (which has no `document` to resolve a relative URL against).
 */
