...
export const META: Record<string, Meta> = {
  ι: { blurb: "The universal combinator — every bird in this forest is built from ι alone.", recipe: "primitive" },
  B: { bird: "Bluebird", blurb: "The forest's composition law: it feeds the result of one function straight into another.", recipe: "S (K S) K" },
  B1: { bird: "Blackbird", blurb: "A Bluebird reaching one argument deeper — composes a function after a three-argument one.", recipe: "B B B" },
  B2: { bird: "Bunting", blurb: "The Bluebird reaching deeper still — composes a function after a four-argument one.", recipe: "B B (B B B)" },
  B3: { bird: "Becard", blurb: "Chains three functions back to back — a composition of compositions.", recipe: "B (B B) B" },
  C: { bird: "Cardinal", blurb: "Flips its next two arguments. On Church Booleans it is logical NOT.", recipe: "S (S (K B) S) (K K)" },
  D: { bird: "Dove", blurb: "A Bluebird that slips one level into a binary function's second argument.", recipe: "B B" },
  ...
  F: { bird: "Finch", blurb: "A permuting bird that reverses the order of three arguments.", recipe: "C V" },
  G: { bird: "Goldfinch", blurb: "Pairs the last argument with a composition of the middle two.", recipe: "B B C" },
  H: { bird: "Hummingbird", blurb: "Duplicates an argument in the middle of a three-way application.", recipe: "B W (B C)" },
  I: { bird: "Identity (Idiot Bird)", blurb: "The do-nothing bird — it answers with exactly the bird it was given.", recipe: "ι ι" },
  J: { blurb: "Hands its third argument to its first and ignores the second.", recipe: "B K T" },
  K: { bird: "Kestrel", blurb: "The constant bird: always answers with its first argument. Doubles as Boolean true and a pair's first projection (fst).", recipe: "primitive" },
  KI: { bird: "Kite", blurb: "Always answers with its second argument. Doubles as Boolean false, the number zero, and a pair's second projection (snd).", recipe: "K I" },
  L: { bird: "Lark", blurb: "Composes a function with self-application — give the forest a Lark and an Identity bird and a Mockingbird is sure to appear.", recipe: "C B M" },
  M: { bird: "Mockingbird", blurb: "The forest's namesake: it echoes its argument back applied to itself — the spark of recursion (and, fed itself, of endless echoing). On Booleans it is logical OR.", recipe: "S I I" },
  M2: { bird: "Double Mockingbird", blurb: "A two-argument Mockingbird — echoes a compound application back to itself.", recipe: "B M" },
  O: { bird: "Owl", blurb: "Feeds a value back through a function — a close relative of the Sage.", recipe: "S I" },
  Q: { bird: "Queer", blurb: "The chief compose-and-shuffle bird: runs the first function, then feeds the result to the second.", recipe: "C B" },
  Q1: { bird: "Quixotic", blurb: "One of the Queer family of reordered compositions: runs the third argument on the second, then the first on that result.", recipe: "B (C B T) B" },
  ...
  R: { bird: "Robin", blurb: "A permuting bird that rotates three arguments, sending the first to the back.", recipe: "C C" },
  S: { bird: "Starling", blurb: "The substitution bird: shares one argument between two functions and applies the results. With the Kestrel it can build every other bird.", recipe: "primitive" },
  T: { bird: "Thrush", blurb: "Reverse application, a close cousin of the Cardinal — it hands its first argument to its second.", recipe: "C I" },
  U: { bird: "Turing", blurb: "Discovered by Alan Turing in 1937: applied to itself it becomes a fixpoint combinator, a fountain of recursion.", recipe: "λx y. y (x x y)" },
  V: { bird: "Vireo", blurb: "Bundles two values into a pair — and a list's cons cell; recover the parts with the Kestrel and the Kite. The key to encoding data.", recipe: "B C (C I)" },
  ...
  Y: { bird: "Sage", blurb: "The bird every other bird is fond of: it hands any function a fixed point of itself, which is exactly what makes recursion possible.", recipe: "B M (C B M)" },
