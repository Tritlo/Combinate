...
export const IOTA_CODE: Record<string, string> = {
  ...
  Q1: "00010101011001010111010010001010101101110010101010110000101010111101001010101011011", // 42ι — smallest known ≤42ι hunt (was 74ι def-expansion)
  Q2: "00100001010101110010101011011000010101011101100101010101101010010101010110110101011", // 42ι — smallest known ≤42ι hunt (was 65ι def-expansion)
  J: "", // 141ι — Rosser's J via B (B C) (W (B C (B (B B B)))); GATE-EXEMPT by design decision: certified equal at 500k caps but unfolds past play budgets, so the canonical picture won't itself trigger discovery if copied exactly (was 158ι)
...
export const IOTA_FASTEST: Record<string, string> = {
  ...
  B2: "000010101011010101011000101010110010101011011010101011001010110101001010101011011", // 191 steps (vs 270)
  ...
  R: "00010101011001010110010101011010101011000101010110010101101010110101011", // 88 steps (vs 547)
  W: "0001010101101010101101011", // 32 steps (vs 33)
  head: "0001010101100010101011011001010110101011001010110101011", // 48 steps (vs 136)
  "Φ": "00010101011001010101011001010110101010110101001010101011011", // 103 steps (vs 124)
};
...
export const CATALOG: Law[] = [
  ...
  bird("M3", "M3 x = x x x", 1, (v) => app(app(v[0], v[0]), v[0])), // Triple Mockingbird — surfaced by the minimal-forms census (): enters the pure-ι universe at exactly 13 ι
...
export const META: Record<string, Meta> = {
  ...
  J: { bird: "Jay", blurb: "The Jay duplicates its first argument deep inside a four-way application, feeding it in twice while reshuffling the rest — a trick it owes to the Warbler and the Bluebird. Smullyan named it the Jay, a J-bird; this is the forest's canonical Jay.", recipe: "B (B C) (W (B C (B (B B B))))" },
