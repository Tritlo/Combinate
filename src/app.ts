...
export async function mountApp(onStep: (label: string) => void = () => {}): Promise<void> {
  ...
  // ---- authoring verbs (): Define (name a subtree → a new block) and
  ...
  function onTreeDown(tree: TreeView, e: FederatedPointerEvent): void {
    ...
    gameInput?.detach(tree); // grabbing a bucket tree with the mouse releases its slot ()
  ...
  // T cycles auto → top-down → radial → square → auto.
  ...
  // ---- game mode (): keyboard/controller play via a bucket tray + hand ----
  let gameMode = false;
  ...
  const menus: Menu[] = [
    ...
    { title: "View", items: [
      ...
      { kind: "radio", label: "H-tree layout", on: () => layoutFn === layoutHTree, run: () => setLayoutMode(layoutHTree) },
      { kind: "toggle", label: "3D view ✦", checked: () => view3D, run: () => toggleView3D() }, // 3D layout mirrors the 2D mode (H-tree → cubic H-tree, else packed sphere)
      { kind: "sep" },
  ...
  window.addEventListener("keydown", (e) => {
    ...
    // A context OWNS the keyboard (): in Build / Inspect the bound keys act and every other
