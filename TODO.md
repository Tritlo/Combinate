...
2. **Write / fill its ** in  (or its own file under 
...
> Modal base; "core ports" dropped per ; app.ts extraction + QuestProgress
...
>
> **Deep perf + reorg push — DONE (`reorg-perf` branch, ):** redexAt spine-rewalk
...
> correctness bugs.
>
> **Turbo (wasm) — DONE (`wasm-turbo` branch, ):** the wasm reducer is now WIRED as a
...
## 1. Optimizations modal — ✅ DONE  ·  (mini)
...
- [ ] **gcd** (`u1Sr43PU`) — Church Euclid is over-budget; **gated on kernels (§5)**:
      native values don't help (raw S/K/I, no named op — ), so it needs a
...
## 4. Quest Mode — a tracked-quest side panel  ·  (mini)
...
- [ ] Spike a MicroHs-style **kernel** mechanism: bind a named combinator to a native JS
      ...
      **Kernels are PURE for now** — a kernel is a pure function of its (evaluated)
      arguments, no IO/effects. (FFI with real effects is a possible future, out of scope.)
       first; consider refactoring `native.ts`'s hardcoded ops onto the registry.

## 6. Reorg / refactor — penultimate  ·  (mini)

Discuss the plan with Codex; think hard about organization, not just the modals.

- [ ] **Ports & adapters — deeper hexagonal ().** Push the functional-core /
