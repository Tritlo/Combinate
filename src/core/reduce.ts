...
export function redexAt(n: Node, argsAbove = 0, fast = false, native?: NativeOpts): Redex | null {
  ...
  // native value op () when recognised, else by its catalog rule (fast mode).
  if (fast || native) {
    const args: Node[] = [];
    let head: Node = n;
    while (head.kind === "app") {
    ...
    if (head.kind === "comb") {
      ...
      // Native value op (): a CHEAP head check during discovery; the expensive
 ...
 * sonification layer (PLAN.md Phase A / ) can pick a tone per reduction
 * without allocating.
 */
