...
/**
 * Drive `onUpdate(progress)` from 0→1 over `ms` on the Pixi ticker (eased). Returns a
 * cancel function. Used for HUD flourishes (toast fade, hotbar slot pop).
 */
export function tween(ticker: Ticker, ms: number, onUpdate: (eased: number) => void): () => void {
  let elapsed = 0;
