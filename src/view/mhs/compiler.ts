...
export class StubCompiler implements Compiler {
  async compile(source: string): Promise<CompileResult> {
    ...
    if (!ex) {
      return {
        error:
          "stub compiler: only the bundled examples compile until the MicroHs→WASM blob lands (, Phase 0). Load an example below — your own source will compile once the real compiler is wired in.",
      };
    }
