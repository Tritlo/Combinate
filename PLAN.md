...
The plan of record for the next arc. Decisions are in –`0008`
(finalised in a grill-with-docs pass); shared language is in `CONTEXT.md`. The
prior PLAN.md (the re-folding lens) shipped in v2–v3 and now lives in 
...
and B3 (the MicroHs *wow*) are deferred and rolled back** — the design stands (
...
MicroHs eval limit, not a resource cap). See  *Deferral* note and the
...
> revival. The wall: the fresh compiler-to-JS build OOMs in self-hosting (see 
...
1. The vendored prebuilt **WASM blob (+ `base.pkg`)** — hosted as a release asset,
   **not** committed to git history (; explicit exception to ).
...
- **`Store` port + DuckDB-WASM adapter** (). A pure `Store` interface in
...
- **Permalink codec** (). Tree (Barker bit-code) + active mode flags ⇄ URL
...
- **Rule-trace from `step()`** (). `step` (or a `stepWithRule`) surfaces the
...
### B1 — Golf, leaderboards, sonification () ✅ shipped
...
### B2 — Authoring: Define, then one-hole Abstract () ✅ shipped
...
### B3 — MicroHs integration () — TS side — DEFERRED
...
1. ~~Merge `roadmap` ( + `CONTEXT.md` + this PLAN) → `main`~~. ✅ done.
