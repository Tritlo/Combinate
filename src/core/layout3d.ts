/**
 * The 3D "packed sphere" layout () — a pure 3D generalization of {@link layoutRadial}.
 ...
 *  rotation ABOUT the diagonal, so the drift never turned. The price (): the one-level
 *  congruence map is now improper (det −1 — chirality alternates per level, as 2D did before
 *  ); two levels apart it is a proper rotation again. In 3D you can't have both: a fixed
